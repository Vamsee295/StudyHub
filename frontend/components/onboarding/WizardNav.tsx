...
export default function WizardNav({ currentStep, handleNext, handleBack, handleSkip, isValid }: WizardNavProps) {
  return (
      ...
      <div className="flex items-center gap-3">
        ...
        {handleSkip && (
          <button
            onClick={handleSkip}
            type="button"
            className="text-xs font-mono font-semibold text-slate-500 hover:text-blue-600 px-3 py-2 rounded-lg hover:bg-slate-100 transition-colors"
          >
            Skip for now →
          </button>
        )}
